...
class Settings:
    ...
    # MARK: Flydigi adaptations
    enable_rumble: bool = True                # drive vibration motors from telemetry
    enable_split_rumble: bool = True         # split HID reports (optimized for Flydigi Apex 4)
    enable_emulation_trigger: bool = True    # automatically spawn MilesMorales.exe in background

    # Telemetry rumble details (Flydigi Apex 4/5 / DInput emulation mode)
    rumble_speed_scale: float = 0.0          # constant rumble based on speed
    rumble_slip_scale: float = 20.0          # rumble intensity when tires are slipping
    rumble_slip_deadzone: float = 0.10       # ignore tire slip below this threshold
    rumble_brake_scale: float = 0.2          # heavy rumble intensity under braking
    rumble_rpm_scale: float = 15.0           # light motor vibration at high engine RPM
    rumble_rpm_threshold: float = 0.80       # RPM ratio threshold to start engine vibration
    rumble_surface_scale: float = 40.0       # heavy motor rumble for road surface bumps
    rumble_surface_deadzone: float = 0.05    # ignore road surface texture below this threshold
