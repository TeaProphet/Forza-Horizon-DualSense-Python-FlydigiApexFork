...
class DualSense:
    ...
    def _build(self, left, right, motor_left: int = 0, motor_right: int = 0, mode: str = "all"):
        ...
        # Critical for rumble on emulated DualSense (Apex 4/5):
        # We must set valid_flag1 to 0xF7 and led_green to 255
        buf[L["flags"] + 1] = 0xF7
        buf[L["flags"] + 45] = 255

        mr_val = max(0, min(255, motor_right))
        ml_val = max(0, min(255, motor_left))
        buf[L["flags"] + 2] = mr_val
        buf[L["flags"] + 3] = ml_val
