...
SYSTEM_SECTIONS = [
    ...
    ("Flydigi Emulation", [
        ("enable_emulation_trigger", "Auto-activate Flydigi DualSense emulation mode", None, None,
         "Spawns a background process that signals Flydigi Space Station to switch to DS Mode."),
    ]),
    ("Startup pulse", [
...
class SettingsTab(ctk.CTkFrame):
    ...
    def _add_switch_row(self, parent, attr: str, label: str, hint: str):
        ...
        if attr == "enable_rumble":
            btn = ctk.CTkButton(row.controls, text=t("Test Rumble"), width=100,
                               command=self.app.test_rumble)
            btn.pack(side="left", padx=(T.PAD_SM, 0))
